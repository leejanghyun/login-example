...
import { showSpinner } from '@/store/features/global';
...
class AxiosApi {
  ...
  private defaultRequestConfig: CommonRequestConfig = {
    headers: {
  ...
  /**
   * Axios의 singleton instance를 가져온다.
   */
  ...
  private async requestInterceptor(requestConfig: CommonRequestConfig): Promise<CommonRequestConfig> {
    ...
    if (!requestConfig.doNotShowSpinner) {
      store.dispatch(showSpinner({ isShowSpinner: true }));
    }
...
const httpClient = AxiosApi.getInstance();
